...
def Choose_File():  # Create function to choose a .db file using the easygui fileopenbox function
    ...
    while not file[-3:].lower() == ".db":
        ...
        if not file[-3:].lower() == ".db":
            # Exit if they don't choose a .db file
            messagebox.showwarning(
                title="File Warning", message="Please choose a .db file, exiting...")
            exit()
        return file
...
# Create fonts for buttons and labels
...
Predict_Button_Photo = PhotoImage(file="Predict Button.PNG").subsample(
    2, 2)  # Get images for buttons
Edit_Button_Photo = PhotoImage(file="Edit Button.PNG").subsample(
    2, 2)  # Get images for buttons

List_Box_Frame_1 = Frame(window, highlightbackground="black",
                         highlightthickness=0, bd=0, width=1)
List_Box_Frame_2 = Frame(window, highlightbackground="black",
                         highlightthickness=0, bd=0, width=1)
# Create frames for the various listboxes
List_Box_Frame_3 = Frame(window, highlightbackground="black",
                         highlightthickness=0, bd=0, width=1)
List_Box_Frame_4 = Frame(window, highlightbackground="black",
                         highlightthickness=0, bd=0, width=1)
...
# Weather label text variable and data
Weather_Label = Label(window, textvariable=Weather_Var, font=Label_Font, bd=0)
...
def Quantify_Data(data):  # Create function to turn data from description into numbers
    ...
    for record in data:
        processed_record = []
        X = [0]*50
        if record[4] == "Heavy Rain":
            X[0] = 1
        if record[4] == "Light Rain":
            # Convert weather to numbers where a zero means it is not those conditions and a one means it is
            X[1] = 1
        if record[4] == "Sunny":
            X[2] = 1
        if record[4] == "Overcast":
            X[3] = 1
        if record[4] == "Fog":
            X[4] = 1
        if record[4] == "Snow":
        ...
        if record[7] == "Tuesday":
            X[19] = 1
        if record[7] == "Wednesday":
            X[20] = 1
        if record[7] == "Thursday":
            X[21] = 1
        if record[7] == "Friday":
            X[22] = 1
        if record[7] == "Saturday":
            X[23] = 1
        if record[7] == "Sunday":
            X[24] = 1
        ...
        for i in range(0, 24):
            if rounded_hour == i:
                # Split the day into 24 hours, where a 1 represents the hour that the run was taken in and the rest are zeroes
                X[25+i] = 1
...
def UnQuantify_Data(data):  # Create a function to turn numerical data back into words
    ...
    for record in data:
        ...
        if record[0][1] == 1:
            X.append("Light Rain")
        if record[0][2] == 1:
            X.append("Sunny")  # Turn the zeroes and ones back into weather
        if record[0][3] == 1:
            X.append("Overcast")
        if record[0][4] == 1:
            X.append("Fog")
        ...
        if record[0][19] == 1:
            X.append("Tuesday")
        if record[0][20] == 1:
            # Turn the zeroes and ones back into the days of the week
            X.append("Wednesday")
        if record[0][21] == 1:
            X.append("Thursday")
        if record[0][22] == 1:
...
def Update_Listbox():  # Update the listbox with the new runs when a run is added, edited or deleted
    ...
    for item in cur.execute('''SELECT * FROM Runs;'''):
        ...
        List_Box_4.insert(counter, str(math.floor(
            item[3]/60))+":"+str(int(round(item[3] % 60))).zfill(2))
        counter += 1
...
# Bind the listboxes to the onselect function
...
def Create_Predict_Window():
    ...
    def Predict_Time():
        Data_Raw = cur.execute('''SELECT * FROM Runs WHERE distance BETWEEN {} AND {};'''.format(
            float(Distance_Var.get())-0.01, float(Distance_Var.get())+0.01))
        Data = Quantify_Data(
            Data_Raw)  # Execute SQL and turn the data into 1s and 0s for regression
        X = []
        Y = []
        for i in Data:
        ...
        Regression = linear_model.LinearRegression().fit(
            X, Y)  # Fit the regression to X and Y variables
        True_Time = int(Hour_Var.get())*60 + \
            int(Minute_Var.get())  # Get the time in seconds

        Quantified_Data = Quantify_Data([[0, ' ', float(Distance_Var.get()), True_Time, Conditions_Clicked.get(), float(
            Temperature_Var.get()), int(Humidity_Var.get()), Day_Clicked.get(), int(Hour_Var.get()), int(Minute_Var.get())]])
        # Turn the data for this specific run into 1s and 0s
        Weather_Var.set("Weather: {}, {}°C, {}% Humidity".format(
            Conditions_Clicked.get(), Temperature_Var.get(), Humidity_Var.get()))
        # Get the predicted time for this specific run
        Prediction = Regression.predict([Quantified_Data[0][0]])[0][0]
        Time_Var.set("Predicted Time: {}:{}".format(math.floor(
            Prediction/60), str(int(Prediction % 60)).zfill(2)))
        Day_Time_Var.set("Day and Time: " + Day_Clicked.get() + " at " +
                         Hour_Var.get() + ":" + Minute_Var.get())  # Update labels and destroy window
        Distance_Var_2.set(
            "Distance: "+str(round(float(Distance_Var.get()), 2))+"km")
    ...
    Predict_Weather_Label = Label(
        Predict_Window, text="Weather", font=Label_Font, bd=0)
    Predict_Conditions_Label = Label(
        Predict_Window, text="Conditions: ", font=Button_Font, bd=0)
    Predict_Temperature_Label = Label(
        Predict_Window, text="Temperature (°C): ", font=Button_Font, bd=0)  # Define labels
    Predict_Humidity_Label = Label(
        Predict_Window, text="Humidity (%): ", font=Button_Font, bd=0)
    Predict_Day_Label = Label(
        Predict_Window, text="Day and Time", font=Label_Font, bd=0)
    Predict_Week_Label = Label(
        Predict_Window, text="Day of Week: ", font=Button_Font, bd=0)
    Predict_Time_Label = Label(
        Predict_Window, text="Time of Day: ", font=Button_Font, bd=0)
    Predict_Predict_Label = Label(
        Predict_Window, text="Run", font=Label_Font, bd=0)
    Predict_Distance_Label = Label(
        Predict_Window, text="Distance: ", font=Button_Font, bd=0)
    Predict_Colon_Label = Label(Predict_Window, text=':', font=Button_Font)

    Predict_Conditions_Drop = OptionMenu(Predict_Window, Conditions_Clicked, "Sunny",
                                         "Heavy Rain", "Light Rain", "Fog", "Overcast", "Snow")  # Define inputs
    Predict_Day_Drop = OptionMenu(Predict_Window, Day_Clicked, "Monday",
                                  "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday")
    Predict_Temperature_Input = Spinbox(
        Predict_Window, textvariable=Temperature_Var, from_=0, to=60)
    Predict_Humidity_Input = Spinbox(
        Predict_Window, textvariable=Humidity_Var, from_=0, to=100)
    Predict_Hour_Input = Spinbox(
        Predict_Window, textvariable=Hour_Var, from_=0, to=23, width=4, format='%02.0f')
    Predict_Minute_Input = Spinbox(
        Predict_Window, textvariable=Minute_Var, from_=0, to=59, width=4, format='%02.0f')
    Predict_Distance_Input = Spinbox(
        Predict_Window, textvariable=Distance_Var, from_=0, to=100, width=10, format='%3.2f', increment=0.01)
    Predict_Predict_Button = Button(Predict_Window, image=Predict_Button_Photo,
                                    bg='#f0f0f0', relief=FLAT, width=150, height=54, bd=0, command=Predict_Time)
...
def delete():
    ...
    cur.execute('DELETE FROM Runs WHERE iD = ' +
                str(List_Box.get(List_Box.curselection())))  # Function to delete runs
    connection.commit()
    Update_Listbox()
...
def Create_Run_Window(add):
    if add:
        Conditions_Clicked = StringVar()
        Day_Clicked = StringVar()
        # Instantiate variables if window is in add mode
        Temperature_Var = StringVar(value=0)
        ...
        Temperature_Var = StringVar(value=Record[5])
    ...
    Run_Weather_Label = Label(
        Run_Window, text="Weather", font=Label_Font, bd=0)
    Run_Conditions_Label = Label(
        Run_Window, text="Conditions: ", font=Button_Font, bd=0)
    Run_Temperature_Label = Label(
        Run_Window, text="Temperature (°C): ", font=Button_Font, bd=0)
    Run_Humidity_Label = Label(
        Run_Window, text="Humidity (%): ", font=Button_Font, bd=0)
    Run_Day_Label = Label(Run_Window, text="Day and Time",
                          font=Label_Font, bd=0)
    Run_Week_Label = Label(Run_Window, text="Day of Week: ",
                           font=Button_Font, bd=0)  # Create labels for the
    Run_Time_Label = Label(
        Run_Window, text="Time of Day: ", font=Button_Font, bd=0)
    Run_Run_Label = Label(Run_Window, text="Run", font=Label_Font, bd=0)
    Run_Distance_Label = Label(
        Run_Window, text="Distance: ", font=Button_Font, bd=0)
    Run_Run_Time_Label = Label(
        Run_Window, text="Time: ", font=Button_Font, bd=0)
    Run_Colon_Label = Label(Run_Window, text=':', font=Button_Font)
    Run_Name_Label = Label(Run_Window, text="Run Name: ",
                           font=Button_Font, bd=0)

    Run_Conditions_Drop = OptionMenu(
        Run_Window, Conditions_Clicked, "Sunny", "Heavy Rain", "Light Rain", "Fog", "Overcast", "Snow")
    Run_Day_Drop = OptionMenu(Run_Window, Day_Clicked, "Monday", "Tuesday",
    ...
    Run_Minute_Input = Spinbox(
        Run_Window, textvariable=Minute_Var, from_=0, to=59, width=4, format='%02.0f')
    Run_Distance_Input = Spinbox(Run_Window, textvariable=Distance_Var,
                                 from_=0, to=100, width=10, format='%3.2f', increment=0.01)
    Run_Time_Min_Input = Spinbox(
        Run_Window, textvariable=Time_Minute_Var, from_=0, to=500, width=4, format='%02.0f')
    Run_Time_Second_Input = Spinbox(
        Run_Window, textvariable=Time_Second_Var, from_=0, to=59, width=4, format='%02.0f')
    Run_Colon_Label_2 = Label(Run_Window, text=':', font=Button_Font)
    if add:
        Run_Add_Button = Button(Run_Window, image=Add_Button_Photo, bg='#f0f0f0',
                                relief=FLAT, width=150, height=54, bd=0, command=Add)
    else:
        Run_Add_Button = Button(Run_Window, image=Edit_Button_Photo, bg='#f0f0f0',
                                relief=FLAT, width=150, height=54, bd=0, command=Edit)
...
def add_window():
    # Create run and edit windows with predefined function
...
def Create_Ideal_Window():
    ...
    def Predict_Ideal():
        weather_types = ["Heavy Rain", "Light Rain",
                         "Sunny", "Overcast", "Fog", "Snow"]
        ...
        for i in Data:
            ...
            Y.append(i[1])
        Regression = linear_model.LinearRegression().fit(
            X, Y)  # Fit regression to X and Y variables

        Coef = list(Regression.coef_[0])  # Get the list of coefficients

        Weather_Coef = Coef[0:5]

        # For each set of coefficients (weather, temperature, humidity, time of day and day of week,
        for i in range(len(Weather_Coef)):
            # find the minimum coefficient (that is the one that lowers the predicted time by the most)
            if Weather_Coef[i] == 0:
                # and create an "ideal" variable with the data point attributed to that coefficient
                Weather_Coef[i] = 100
        ...
        Ideal_Temperature = (Temperature_Coef.index(
            min(Temperature_Coef))+1)*5 - 2.5
        ...
        Weather_Ideal_Var.set("Weather: {}, {}°C, {}% Humidity".format(
            Ideal_Weather, Ideal_Temperature, Ideal_Humidity))  # Update weather label
        ...
        Day_Time_Ideal_Var.set("Day and Time: {} at {}:00".format(
            Ideal_Day, Ideal_Time))  # Update day and time labels
    ...
    Ideal_Distance_Label = Label(
        Ideal_Window, text="Run Distance ", font=Label_Font, bd=0)
    Ideal_Distance_Label_2 = Label(
        Ideal_Window, text="Distance: ", font=Button_Font, bd=0)  # Create Labels

    Ideal_Distance_Input = Spinbox(Ideal_Window, textvariable=Distance_Var,
                                   from_=0, to=100, width=10, format='%3.2f', increment=0.01)
    Ideal_Predict_Button = Button(Ideal_Window, image=Predict_Button_Photo, bg='#f0f0f0',
                                  relief=FLAT, width=150, height=54, bd=0, command=Predict_Ideal)  # Create button and input
...
Scrollbox = Frame(window, highlightbackground="black",
                  highlightthickness=0, bd=0, width=10, height=1000)

scrollbar = Scrollbar(Scrollbox, orient=VERTICAL)
# Create a border for the time predict button and the button itself
Time_Button_Border = Frame(
    window, highlightbackground="black", highlightthickness=2, bd=0)
Time_Button = Button(Time_Button_Border, text="Predict Time From Conditions", font=Button_Font,
                     bg='#edead9', relief=FLAT, bd=10, width=22, command=Create_Predict_Window)

# Create a border for the conditions predict button and the button itself
Ideal_Button_Border = Frame(
    window, highlightbackground="black", highlightthickness=2, bd=0)
Ideal_Button = Button(Ideal_Button_Border, text="Predict Ideal Conditions", font=Button_Font,
                      bg='#edead9', relief=FLAT, bd=10, width=22, command=Create_Ideal_Window)


Day_Time_Var = StringVar()
# Day and time label text variable and data
Day_Time_Label = Label(window, textvariable=Day_Time_Var,
                       font=Label_Font, bd=0)
Day_Time_Var.set("Day and Time: ")

Distance_Var_2 = StringVar()
# Distance label text variable and data
Distance_Label = Label(
    window, textvariable=Distance_Var_2, font=Label_Font, bd=0)
Distance_Var_2.set("Distance: ")

Time_Var = StringVar()
# Predicted time label text variable and data
Time_Label = Label(window, textvariable=Time_Var, font=Label_Font, bd=0)
Time_Var.set("Predicted Time: ")


Ideal_Condition_Var = StringVar()
# Weather label text variable and data
Ideal_Condition_Label = Label(
    window, textvariable=Ideal_Condition_Var, font=Label_Font, bd=0)
Ideal_Condition_Var.set("Ideal Conditions")

Weather_Ideal_Var = StringVar()
# Day and time label text variable and data
Weather_Ideal_Label = Label(
    window, textvariable=Weather_Ideal_Var, font=Label_Font, bd=0)
Weather_Ideal_Var.set("Weather: ")


Day_Time_Ideal_Var = StringVar()
# Predicted time label text variable and data
Day_Time_Ideal_Label = Label(
    window, textvariable=Day_Time_Ideal_Var, font=Label_Font, bd=0)
Day_Time_Ideal_Var.set("Day and Time: ")


# Create a border to surround all the edit, add and delete buttons
Edit_Add_Delete_Border = Frame(
    window, highlightbackground="black", highlightthickness=1, bd=0)

# Create a border for each edit, add and delete button
Edit_Button_Border = Frame(
    Edit_Add_Delete_Border, highlightbackground="black", highlightthickness=1, bd=0)
Add_Button_Border = Frame(
    Edit_Add_Delete_Border, highlightbackground="black", highlightthickness=1, bd=0)
Delete_Button_Border = Frame(
    Edit_Add_Delete_Border, highlightbackground="black", highlightthickness=1, bd=0)


Edit_Button = Button(Edit_Button_Border, text="Edit", font=Button_Font,
                     bg='#b5d9fe', relief=FLAT, bd=5, width=7, command=edit_window)
# Create Edit, Add and Delete buttons                        #Create the edit, add and delete buttons
Add_Button = Button(Add_Button_Border, text="Add", font=Button_Font,
                    bg='#b5d9fe', relief=FLAT, bd=5, width=7, command=add_window)
Delete_Button = Button(Delete_Button_Border, text="Delete", font=Button_Font,
                       bg='#f9c7c7', relief=FLAT, bd=5, width=7, command=delete)


Edit_Button_Border.grid(row=0, column=0)
Add_Button_Border.grid(row=0, column=1)
Delete_Button_Border.grid(row=0, column=2)
# Place edit, add and delete buttons
...
# Set each list box to be based off the same scroll bar
...
def multiple_yview(*args):
    List_Box.yview(*args)
    # Change the yview (scroll amount) of all listboxes with one function
